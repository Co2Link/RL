...
# 换成GPU之后，learn时间大大缩减为0.1秒左右，但choose_action需要消耗1ms,每episode大概数千step，所以大部分时间消耗在choose_action上
...
class Net(nn.Module):
    def __init__(self):
        ...
        if not discrete:
            self.logstd = torch.nn.Parameter(torch.tensor(np.random.normal(0, 0.3, N_ACTIONS),dtype=torch.float32))
...
class PolicyGradient(object):
    def __init__(
        ...
        assert torch.cuda.is_available() # change
        self.net=Net().cuda() # change
    ...
    def choose_action(self, x):
        x = torch.unsqueeze(torch.FloatTensor(x), 0).cuda() # change
        act_logits = self.net.forward(x)
        if self.discrete:
            act_prob = F.softmax(act_logits)
            action = np.random.choice(range(act_prob.size()[1]), p=act_prob.cpu().detach().numpy().ravel())
        else:
            mean = act_logits
            logstd = self.net.logstd.expand_as(mean)  # ( N_ACTIONS,) -> ( Batch_size, N_ACTION)
            action = torch.normal(mean, torch.exp(logstd)).cpu().detach().numpy().ravel()  # mean shoud have the same shape with std
        return action
    ...
    def learn(self):
        ...
        pyt_obs=torch.FloatTensor(self.ep_obs).cuda() # change
        pyt_rs=torch.unsqueeze(torch.FloatTensor(discounted_ep_rs_norm),1).cuda() # change

        # calculate the loss
        if discrete:
            pyt_as = torch.unsqueeze(torch.LongTensor(self.ep_as), 1).cuda() # change
            act_logits = self.net(pyt_obs)
            act_prob = F.softmax(act_logits).gather(1, pyt_as)  # (batch,1)
            neg_log_prob = -torch.log(act_prob)
        else:
            pyt_as = torch.unsqueeze(torch.FloatTensor(self.ep_as), 1).cuda() # change
        ...
        self.ep_steps_list=[]
        print('before',torch.cuda.memory_cached()/2**30)
        print('before',torch.cuda.memory_allocated()/2**30)
        torch.cuda.empty_cache() # 解决内存溢出
        print('after',torch.cuda.memory_cached()/2**30)
        print('after',torch.cuda.memory_allocated()/2**30)
